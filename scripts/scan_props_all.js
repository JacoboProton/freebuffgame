...
if (!fs.existsSync(root)) {
  console.error('apps/web/src not found');
  process.exit(2);
}
...
for (const file of files) {
  ...
  while ((m = propRx.exec(txt)) !== null) {
    ...
    const suspectKeywords = ['prisma', 'PrismaClient', 'new Date', 'Date(', 'Request', 'Response', 'clerk', 'getServerSession', 'getUser', 'user', 'auth', 'cookies', 'headers', 'payment', 'purchase', 'order', 'session', 'ctx'];
