'use client';

import { useSearchParams } from 'next/navigation';
import { Suspense, useEffect } from 'react';
import { Button } from '@/components/ui/button';
import { AlertCircle } from 'lucide-react';

interface GoogleAuthButtonProps {
  mode: 'signin' | 'signup';
}

function GoogleAuthContent({ mode }: GoogleAuthButtonProps) {
  const searchParams = useSearchParams();
  const error = searchParams.get('error');

  useEffect(() => {
    if (searchParams.get('google_auth') === 'success') {
      // Reload to sync Clerk authentication state with the JWT cookie
      window.location.href = '/dashboard';
    }
  }, [searchParams]);

  const handleGoogleAuth = () => {
    const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
    window.location.href = `${baseUrl}/api/auth/google`;
  };

  const isSignUp = mode === 'signup';
  const buttonText = isSignUp ? 'Registrarse con Google' : 'Iniciar sesión con Google';
  const dividerText = isSignUp ? 'o regístrate con' : 'o continúa con';
  const errorText = isSignUp 
    ? 'Error al registrarse con Google. Por favor intenta de nuevo.'
    : 'Error al iniciar sesión con Google. Por favor intenta de nuevo.';

  return (
    <>
      {/* Google OAuth Error Alert */}
      {error === 'google_failed' && (
        <div className="mb-4 p-4 bg-error/10 border border-error/20 rounded-lg flex items-center gap-3">
          <AlertCircle className="w-5 h-5 text-error flex-shrink-0" />
          <p className="text-sm text-error">{errorText}</p>
        </div>
      )}

      {/* Divider */}
      <div className="relative my-4">
        <div className="absolute inset-0 flex items-center">
          <div className="w-full border-t border-gray-200"></div>
        </div>
        <div className="relative flex justify-center text-sm">
          <span className="px-2 bg-background text-muted-foreground">{dividerText}</span>
        </div>
      </div>

      {/* Google Auth Button */}
      <Button
        onClick={handleGoogleAuth}
        variant="outline"
        size="lg"
        className="w-full gap-2"
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        {buttonText}
      </Button>
    </>
  );
}

export function GoogleAuthButton(props: GoogleAuthButtonProps) {
  return (
    <Suspense fallback={<div className="animate-pulse">Cargando...</div>}>
      <GoogleAuthContent {...props} />
    </Suspense>
  );
}