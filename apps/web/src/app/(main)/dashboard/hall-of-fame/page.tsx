...
import { ArrowLeft, Trophy, Sparkles, Star, Calendar, Users, Award, Clock, Crown, Flame, Share2, Copy, Check } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { ConfettiCelebration } from '@/components/jac-mascot';
import { cn } from '@/lib/utils';
import { handleShare, shareToTwitter, shareToWhatsApp, shareToFacebook, copyShareLink } from '@/lib/share-utils';
...
interface HallOfFameEntry {
  ...
  achievements: LegendaryAchievement[];
...
export default function HallOfFamePage() {
  ...
  const fetchHallOfFame = async () => {
    try {
      ...
      if (result.status === 'success') {
        setData(result.data);
        const isLegendary = result.data.hallOfFame?.some(
          (e: HallOfFameEntry) => e.isCurrentUser
        );
        if (isLegendary && !hasTriggered) {
          setTimeout(() => {
  ...
  const getRankBg = (rank: number) => {
    switch (rank) {
      case 1:
        return 'bg-gradient-to-r from-yellow-50 via-amber-50 to-yellow-50 border-yellow-300 shadow-lg';
      case 2:
  ...
  const currentUserEntry = data?.hallOfFame?.find(e => e.isCurrentUser);
            ...
            <div className="relative rounded-2xl overflow-hidden">
              ...
              <div className="relative bg-gradient-to-r from-yellow-50 via-amber-50 to-orange-50 border-2 border-yellow-400 rounded-2xl p-6 text-center">
                ...
                <h3 className="text-xl font-bold text-amber-800 mb-1">
                  ¡Estás en el Hall of Fame! 🏛️
                </h3>
                <p className="text-sm text-amber-700">
                  Has desbloqueado {currentUserEntry.totalLegendaryCount} logro{currentUserEntry.totalLegendaryCount > 1 ? 's' : ''} legendario{currentUserEntry.totalLegendaryCount > 1 ? 's' : ''} — tu leyenda está asegurada.
                </p>
                <button onClick={() => handleShare(currentUserEntry)} className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-full text-sm font-semibold hover:from-amber-600 hover:to-orange-600 transition-all shadow-md">
                  <Share2 className="w-4 h-4" /> Compartir mi posición
                </button>
              </div>
            </div>
          </motion.div>
              ...
              <CardContent className="p-6 text-center">
                ...
                >
                  <Avatar className="w-24 h-24 mx-auto border-4 border-yellow-400 shadow-lg mb-3">
                    ...
                    <AvatarFallback className="bg-gradient-to-br from-yellow-400 to-amber-500 text-white text-4xl font-bold">
                ...
                <p className="text-sm text-gray-500 mb-3">
                  Nivel {topUser.level} • {topUser.xp.toLocaleString()} XP
                </p>
                <div className="flex items-center justify-center gap-2 mb-3">
                  <Badge className="bg-yellow-100 text-yellow-700 border-yellow-200">
                    <Flame className="w-3 h-3 mr-1" />
                    {topUser.totalLegendaryCount} logros legendarios
                  </Badge>
                  <Badge className="bg-gray-100 text-gray-600 border-gray-200">
                    <Calendar className="w-3 h-3 mr-1" />
                    Desde {formatDate(topUser.firstLegendaryAt)}
                  </Badge>
                </div>
                  ...
                  {topUser.achievements.map((ach) => (
                    <div
                      ...
                      className={cn(
                        ...
                        ACHIEVEMENT_BG[ach.key] || 'bg-gray-50 border-gray-200'
                      )}
                    >
                      <span>{ach.icon}</span>
                      <span>{ach.title}</span>
          ...
          <Card className="overflow-hidden">
            <div className="bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 px-6 py-4 text-white">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-lg">Hall of Fame — Ranking Legendario</h2>
              </div>
            </div>
              ...
              {!data?.hallOfFame || data.hallOfFame.length === 0 ? (
                <div className="px-6 py-16 text-center">
                  ...
                  <h4 className="text-xl font-semibold text-gray-700 mb-2">
                    Sé el primero en entrar al Hall of Fame
                  </h4>
                  <p className="text-gray-500 max-w-md mx-auto mb-6">
                    Desbloquea un logro legendario para ganar tu lugar entre las leyendas de la plataforma
                  </p>
                  <Link
                    href="/courses"
                ...
                data.hallOfFame.map((entry, index) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={cn(
                          "px-6 py-4 flex items-center gap-4 transition-all cursor-pointer hover:bg-gray-50/50",
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {entry.totalLegendaryCount} logro{entry.totalLegendaryCount > 1 ? 's' : ''}
                            </span>
                            <span>•</span>
                            <span className="flex items-center gap-1">
                              <Clock className="w-3 h-3" />
                              {formatDate(entry.firstLegendaryAt)}
                            </span>
                          </div>
                              ...
                              {entry.achievements.map((ach) => (
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <div className="font-semibold text-sm text-gray-800">{ach.title}</div>
                                      <div className="text-xs text-gray-500">{ach.description}</div>
                                    </div>
                                  </div>
                                  <div className="text-right">
                                    <div className="text-xs text-gray-400">{formatDateTime(ach.unlockedAt)}</div>
                                    <div className="text-xs font-semibold text-purple-600">+{ach.xpReward} XP</div>
                                  </div>
                                </div>
                              ))}
                            </div>
            ...
            <div className="px-6 py-4 bg-gradient-to-r from-yellow-50 to-amber-50 text-center border-t border-amber-100">
              <p className="text-xs text-amber-700 font-medium flex items-center justify-center gap-1.5">
                ...
                Ordenados por número de logros legendarios — los más legendarios quedan arriba
                <Sparkles className="w-3 h-3" />
              </p>
            </div>
          </Card>
